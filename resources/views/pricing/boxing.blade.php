...
<div class="page-transition">
    ...
    <section class="relative w-full min-h-screen text-gray-800">
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-3 sm:px-6 py-8 sm:py-16">
            <!-- Title -->
            <h1 class="text-2xl sm:text-3xl md:text-5xl font-bold text-white text-center mb-4 sm:mb-6">Boxing Membership Plans</h1>
            <p class="text-base sm:text-lg text-white text-center mb-6 sm:mb-10">Train with professional boxing coaches in our dedicated boxing facilities</p>
            ...
            <div class="flex justify-center mb-10 w-full overflow-x-auto pb-2 px-2 snap-x scrollbar-hide">
                <div class="flex space-x-2 md:space-x-4 min-w-max mx-auto">
                    ...
                    <a href="{{ route('pricing.boxing') }}" class="px-4 py-2 rounded-md font-semibold bg-gray-300 text-black whitespace-nowrap snap-start">
                        Boxing
                    </a>
            ...
            <div class="flex flex-wrap justify-center gap-4 md:gap-8">
                ...
                <div class="w-full sm:w-80 bg-white rounded-2xl shadow-lg p-6 flex flex-col border-2 border-blue-600 card-animation">
                    ...
                    <div class="flex justify-between items-center mt-auto">
                        ...
                        @if(Auth::check())
                            @if($userHasActive)
                                @if($activeType == 'boxing' && $activePlan == 'monthly')
                ...
                <div class="w-full sm:w-80 bg-white rounded-2xl shadow-lg p-6 flex flex-col card-animation">
                    ...
                    <ul class="text-sm text-gray-700 space-y-2 mb-6">
                        ...
                        <li class="flex items-start gap-2">
                            <x-check-icon /> Boxing equipment provided
                        </li>
                    </ul>
                    <div class="flex justify-between items-center mt-auto">
                        <span class="text-lg font-semibold text-gray-800">₱260 / session</span>
                        @if(Auth::check())
                            @if($userHasActive)
                                @if($activeType == 'boxing' && $activePlan == 'per-session')
                                    <div class="flex flex-col items-end">
                                        <span class="text-sm px-3 py-1 bg-green-100 text-green-600 rounded-full">Current Plan</span>
                                    </div>
                                @else
                            ...
                            @elseif($userRole === 'trainer')
                                <span class="text-sm px-3 py-1 bg-gray-100 text-gray-600 rounded-full">Trainer</span>
                            @else
                                <button onclick="openWaiverModal('boxing', 'per-session', '260.00')" class="text-sm px-3 py-1 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition">
                                    Join Now
                                </button>
                            @endif
                        @else
