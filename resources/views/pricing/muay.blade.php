        ...
        <div class="flex flex-wrap justify-center gap-8">
            ...
            <div class="w-80 bg-white rounded-2xl shadow-lg p-6 flex flex-col border-2 border-blue-600">
                ...
                <div class="flex justify-between items-center mt-auto">
                    <span class="text-lg font-semibold text-gray-800">₱3,200 / Monthly</span>
                    @if(Auth::check())
                        @if($userHasActive)
                            <span class="text-sm px-3 py-1 bg-gray-100 text-gray-600 rounded-full">Active</span>
                        @else
                            <button onclick="openWaiverModal('muay', 'monthly', '150.00')" class="text-sm px-3 py-1 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition">
                                Subscribe Now
                            </button>
                        @endif
                    @else
            ...
            <div class="w-80 bg-white rounded-2xl shadow-lg p-6 flex flex-col">
                <span class="inline-block text-xs font-semibold text-gray-500 mb-2">Membership</span>
                <h2 class="text-xl font-bold mb-3">10 Sessions Pass</h2>
                <ul class="text-sm text-gray-700 space-y-2 mb-6">
                <li class="flex items-start gap-2">
                ...
                <div class="flex justify-between items-center mt-auto">
                    <span class="text-lg font-semibold text-gray-800">₱2,600 / sessions</span>
                    @if(Auth::check())
                        <button onclick="openWaiverModal('muay', 'per-session', '20.00')" class="text-sm px-3 py-1 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition">
                            Buy Now
                        </button>
                    @else
                        <a href="{{ route('login') }}" class="text-sm px-3 py-1 bg-gray-300 text-gray-700 rounded-full hover:bg-gray-400 transition">
